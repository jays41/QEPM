...
def get_fundamental_data():
    ...
    def get_pegy(data):
        try:
            pegy = data['PEG_trailing'] / data['divyield']
        except:
            pegy = None
    ...
    for i in range(len(df)):
        ...
        new_row = {
            ...
            'price-sales': row['ps'],
            'price-earnings-growth': row['PEG_trailing'], # row['peg_ltg_forward'] or row['peg_1yr_forward'] or row['peg_trailing'], # multiple values available - not sure which we want to use
            ...
            'free-cash-operating-cash': row['fcf_ocf']
        }
