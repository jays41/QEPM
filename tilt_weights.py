...
def tilt_function(factor_average):
    """
    Tilt function: 2 / (1 + exp(-c * x))
    Uses piecewise_c to define `c` based on the factor_average.
    """
    c = 0.5

    if factor_average > np.log(pow(3,(1/c))):
        factor_average = 0

    return (2 / (1 + np.exp(-c * factor_average)))



def get_tilted_weights(stock_data, expected_returns, momentum_average, cov_matrix, betas, taus, sectors_array, target_annual_risk):
    ...
    # Compute tilt values using external function
    tilt_aggregate = tilt_function(momentum_average)

    # Boolean masks for long and short stocks
    long_mask = w > 0
    short_mask = w < 0

    # Extract taus for long and short stocks
    taus_long = cp.multiply(long_mask, taus)  # Taus for long stocks
    taus_short = cp.multiply(short_mask, taus)  # Taus for short stocks

    # Compute weighted sum of taus for long and short portfolios
    long_tilt = cp.sum(cp.multiply(cp.pos(w), taus_long))
    short_tilt = cp.sum(cp.multiply(cp.neg(w), taus_short))

    # Implement tilting constraint
    constraints.append(cp.norm(tilt_aggregate * long_tilt - short_tilt, 2) <= epsilon)

# A possible problem:
# We are regressing the taus only on returns without any other covariates, meaning they can be biased if highly correlated with other factors in the model, 
# --> creating unwanted exposure to other factors

# Improvement:
# Use a per-stock constraint on the betas, and regress the taus with other factors included in the model
